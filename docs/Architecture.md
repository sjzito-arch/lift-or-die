...
## ADR-026 — One shared set-progression renderer, not five parallel implementations

- Date: 2026-07-27
- Status: Accepted
- Decision: `js/setProgression.js` exports a single `setProgressionMarkup(exercise)`, purely derived from `setResults.length` vs. `targetSets`/`targetReps` (no separate "which box" state), used identically by `activeExercise.js`, `rest.js`, `exerciseTransition.js`, `workoutCompletion.js`, and `history.js`. It replaces the completion-only `.set-chips`/`setChipsMarkup` built in an earlier pass, which is now deleted.
- Reason: Requirement to show set-by-set progress consistently everywhere, with a hard constraint not to build separate implementations of the same logic. Deriving purely from data (not a separately-tracked "current box index") keeps it consistent with the rest of the app's screen-dispatch philosophy (ADR-009) — the row is automatically correct after Set Done, Partial Set, or Undo, and after a reload, with no extra bookkeeping.
- Alternatives considered: Extending the existing completion-only chip markup in place per-screen; rejected — that's exactly the "separate implementations" the requirement ruled out, and it would have left History's plain-text set summary (`formatSetSummary`, now deleted) as a sixth, inconsistent representation.
- Tradeoffs: None. Verified directly across all five screens, including Deadlift's 1-set configuration (a single box, not a hardcoded five) and a reload mid-exercise (row matches pre-reload state exactly).

## ADR-027 — Exercise-transition screen: absolute per-side load is dominant again, diff is supporting

- Date: 2026-07-27
- Status: Accepted (supersedes the transition-screen portion of the "distance readability" pass, 2026-07-25)
- Decision: On the exercise-transition screen's "Next" block, the upcoming exercise's absolute per-side weight (via the shared `weightDisplayMarkup`) is the visually dominant line again; the add/remove/no-change diff text and the bar+total breakdown are both small, supporting lines beneath it.
- Reason: An earlier pass made the diff text dominant instead, on the reasoning that "what changes" was the most actionable figure. Direct product-owner feedback correctly identified that the *absolute* per-side number is what's actually needed at the rack — for changing plates on the same bar, or loading a completely different bar from scratch, where "remove 10 lb" alone isn't enough to act on without already knowing the starting point. This is a deliberate reversal of a prior explicit decision, not a bug fix; both are recorded so the history of *why* the hierarchy changed twice is legible.
- Alternatives considered: Showing both figures at equal size; rejected as diluting the "one dominant number" principle the rest of the distance-readability work depends on.
- Tradeoffs: None. Verified directly: per-side renders at the same dominant size used on the ready/rest screens, diff and breakdown render at the small supporting size.

## ADR-028 — Easy result: doubles the increment, not a separate hard-coded jump

- Date: 2026-07-27
- Status: Accepted
- Decision: A fully successful exercise can be marked **Easy** (exercise-transition screen for non-final exercises, workout completion review for every eligible exercise including the final one). `computeSuggestedWeight` becomes `targetWeight + increment × (easy ? 2 : 1)`. Enforced in `session.js`'s new `setExerciseEasy()` (throws if `success !== true`, not just hidden in the UI) as well as in the render layer. `undoLastSet` and History's edit-save flow both force `easy: false` the instant `success` stops being `true`, so a stale Easy mark can never survive its own eligibility.
- Reason: Spec-directed: "adding a configured increment × 2," not an arbitrary fixed bonus, so Easy scales correctly with whatever increment each exercise is already configured with (a 2.5 lb increment doubles to 5, not to some unrelated flat number). Enforcing eligibility in the data layer too (not just a hidden checkbox) closes a real gap — a stale in-flight request from a screen the user has since navigated away from could otherwise mark an exercise no longer complete.
- Alternatives considered: A flat "+X lb bonus" regardless of the exercise's own increment; rejected — directly contradicts the spec's explicit doubling rule and would produce a nonsensical jump for exercises with a small custom increment.
- Tradeoffs: `workoutCompletion.js` tracks manual weight-field overrides in a small module-level map keyed by session id (`manualOverridesBySession`), outside the render function, specifically so a full re-render triggered by toggling Easy doesn't clobber something the user already typed — this is slightly more state than a fully stateless render, but it's the only way to make "Easy updates the suggestion unless overridden" both correct and predictable across re-renders. Verified directly: a manual override survives an Easy toggle; Easy without an override updates the suggested value; Undo clears Easy the instant eligibility is lost, confirmed both in the UI and directly against IndexedDB.

## ADR-029 — Unified content-repetition system: family keys, value-keyed dynamic content, 3-workout lookback

- Date: 2026-07-27
- Status: Accepted (supersedes ADR-018's "allow repeats once the pool is exhausted" fallback); corrected by ADR-030 the same week — this ADR's own text below still describes the *intent* correctly, but the initial implementation applied family-based exclusion to dynamic content across workouts too, which ADR-030 fixes.
- Decision: Rest cards and the one-time workout-completion humorous headline now share one repetition system in `js/restCards.js`. Every candidate carries a `key` (unique) and a `family` (shared by variations of the same joke/idea — e.g. every "the equipment doesn't care about your complaints" joke shares one family, capping how often that idea as a whole can surface, not just its exact wording). Value-keyed dynamic content (progress-vs-history cards, the upcoming-exercise card) includes the actual displayed numbers in its key, so a genuinely different number is treated as fresh content even when the template repeats — its `family` stays keyed to just the exercise/metric, so within one workout only one variation of "progress on this exercise" surfaces at a time regardless of the number. `StoredWorkout.contentHistory` (an array of `{key, family}`, new field, absent-and-ignored on older records) records everything shown in a *completed* workout; a fresh pick excludes anything shown this workout (`session.shownCardKeys`, same shape) plus anything in the last 3 completed workouts' `contentHistory`. If nothing is eligible, the pick returns `null` and the screen shows nothing — no repeat fallback.
- Reason: Explicit requirement to stop allowing repeats once a workout's own pool was exhausted (ADR-018's original behavior), extend the exclusion window across the next 3 *completed* workouts, unify rest cards and the completion headline as one system (a joke can't appear twice across those two surfaces in the same workout), and — critically — not let genuinely-changed personal data (e.g. a new working weight) get blocked by a stale key tied only to the template. The headline specifically moved from a separate, untracked 3-item list (`statsCalculations.js`'s old `pickHumorousHeadline`, now deleted) into this same system, drawing from the same humor pool.
- Alternatives considered: Storing only bare keys in history and re-deriving family from the current code's candidate pool at exclusion-check time; rejected — a historical key's content may have been rebalanced or removed from the library by the time it's checked, so the family wouldn't reliably resolve. Storing family alongside key at write-time avoids ever needing that reconstruction. A single shared "no repeat ever" key for all variants of dynamic content; rejected — would block e.g. a real, different progress number from ever showing again within the exclusion window, contradicting the explicit "reuse the format when the data meaningfully differs" requirement.
- Tradeoffs: `session.shownCardKeys` may contain a handful of legacy bare-string entries if a session was already active across this update's deployment; both `restCards.js` and `session.js` normalize those to their own single-item family on read, so exclusion logic never has to special-case the shape. Verified directly: 30 repeated picks against a seeded 9-item history correctly excluded every historical key *and* every other member of an excluded item's family (confirmed the "barbell complaint" family blocked related jokes it hadn't itself shown), and the completion headline was drawn from and recorded into the exact same history.

## ADR-030 — Dynamic content is excluded across workouts by key only, never by family

- Date: 2026-07-27
- Status: Accepted (corrects ADR-029)
- Decision: Every content candidate now carries an explicit `dynamic` boolean (`true` for the value-keyed progress/original-vs-current/upcoming-exercise cards; absent/`false` for everything else). `buildHistoricalExclusions()` only adds a historical entry's `family` to the cross-workout exclusion set when `!entry.dynamic`; `pickFromPool` mirrors this — a dynamic candidate is only ever excluded by its own exact `key` (this workout or the last 3 completed), never by `family`. `dynamic` is persisted alongside `key`/`family` in both `session.shownCardKeys` and `StoredWorkout.contentHistory` so the exclusion check never has to guess it from old data.
- Reason: ADR-029's original implementation kept dynamic content's `family` generic (e.g. `progress:votes`, deliberately not including the vote count) so its *within-workout* role stayed simple, but then reused that same generic `family` for the *cross-workout* lookback too — meaning a stale `progress:votes:10` shown two workouts ago blocked the family `progress:votes` entirely, silently preventing `progress:votes:11` (a genuinely different, correct number) from ever showing until the old entry aged out. The fix is to stop conflating "groups variations of one idea" (what family means for a joke) with "groups instances of one metric" (what family accidentally became for dynamic content) — they need different exclusion rules, not the same field doing two jobs.
- Alternatives considered: Giving dynamic content a unique, unshared family per value (defeating the purpose of having a family field at all for that content); rejected as pointless indirection when "exclude by key only" says the same thing directly. Removing `family` from dynamic candidates entirely; rejected — still useful for readability/debugging even though it's not consulted for exclusion.
- Tradeoffs: None. Verified directly: an identical dynamic value (same vote count) stayed excluded across a simulated 3-workout history; a changed value (vote count +1) was correctly eligible despite sharing a family with the excluded entry; static family exclusion (a "barbell complaint" family member, a legacy pre-`dynamic` entry's family) was unaffected and still worked.

## ADR-031 — `pickFromPool` weight is always defensively resolved, never trusted raw

- Date: 2026-07-27
- Status: Accepted
- Decision: `pickCompletionHeadline` now assigns each candidate an explicit `weight: 1` before calling the shared picker (`buildHumorCards()` alone never sets one — that's normally added by `pickNextCard`'s own per-category weighting, which `pickCompletionHeadline` doesn't go through). Independently, `pickFromPool` itself now resolves every weight through `safeWeight()`, which falls back to `1` for anything that isn't a finite positive number, rather than using `card.weight` directly in the total/roll arithmetic.
- Reason: Passing weightless cards into `pickFromPool` made every `sum + card.weight` term `NaN`, so the running total was `NaN`, the roll was `NaN`, and `roll <= 0` is false for every comparison against `NaN` — the loop never returned early and fell through to `return available[available.length - 1]`, deterministically the last eligible item, not a random pick. Fixing only the call site (`pickCompletionHeadline`) would have resolved today's bug but left the shared picker itself trusting an unvalidated input from any future caller; `safeWeight()` closes that off at the source so the same class of bug can't recur silently.
- Alternatives considered: Only fixing `pickCompletionHeadline`'s call site; rejected — leaves `pickFromPool` fragile against the exact same mistake in a future caller, and the task explicitly called for the shared picker to be made defensive, not just the one broken call site patched.
- Tradeoffs: None. Verified directly: 25 draws against a full, unconstrained humor pool produced 8 distinct headlines (not the same one every time, and not always the pool's last item); the empty-pool and humor-off cases still correctly return `null` rather than picking anything.
